...
export default function KioskPage() {
  ...
  const { data: config, isLoading: configLoading } = trpc.kiosk.bySlug.useQuery(
    { slug },
    { enabled: !!slug },
  )
    ...
    return (
      <div className="flex h-screen w-full items-center justify-center bg-[#131314] text-[#e3e3e3]">
        <div className="animate-pulse space-y-4 text-center">
          <Monitor className="mx-auto size-8 text-zinc-600" />
          <p className="text-sm text-zinc-500">Loading kiosk...</p>
        </div>
      </div>
    )
  }
  ...
  if (!config) {
    return (
      <div className="flex h-screen w-full flex-col items-center justify-center bg-[#131314] text-[#e3e3e3] gap-4">
        <Monitor className="size-12 text-zinc-600" />
        <h1 className="text-xl font-semibold">Kiosk Not Found</h1>
        <p className="text-sm text-zinc-400 max-w-md text-center">
          The kiosk at this URL does not exist or has not been configured yet.
        </p>
        <Link
          href="/settings/kiosk"
          className="mt-2 inline-flex items-center gap-2 rounded-xl bg-white/10 px-4 py-2 text-sm text-white hover:bg-white/20 transition-colors"
        >
          Go to Settings
        </Link>
      </div>
    ...
    <div className="flex h-screen w-full flex-col justify-between bg-[#131314] text-[#e3e3e3] font-sans overflow-hidden antialiased select-none">
      ...
      <header className="flex items-center justify-between px-8 py-5">
        <div className="flex items-center gap-2.5">
          ...
          <span className="ml-2 text-xs text-zinc-600 font-mono">/{slug}</span>
        </div>
      ...
      <footer className="py-4 text-center text-xs text-zinc-600 border-t border-zinc-950 bg-[#131314]">
        Press Enter to submit &bull; Press Esc to clear entry
      </footer>
